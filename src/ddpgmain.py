...
class DDPG(object):
    ...
    def __init__(self, action_dim, state_dim, action_range, replay_buffer):
        ...
        def get_critic(input_state_shape, input_action_shape, name=''):
            ...
            在DDPG中我们把 状态和动作同时输入到 Critic 网络中，去估计Q(s,a)。所以定义两个输入层，然后连接起来，最后模型的输入部分定义两个输入。
            """
    ...
    def learn(self):
        """
        Update parameters——actor和critic的更新
        :return: None
        """
        self.var *= .9995
        ...
        """
        Critic部分的更新和DQN是一样的，使用td-error来更新。用目标网络构造 target，然后和当前网络输出的q计算MSE损失，然后更新网络参数。
        """
        ...
        """
        DDPG采用梯度上升法，Actor作用就是输出一个动作，这个动作输入Critic网络能得到最大的Q值。由于和梯度下降方向相反，所以需要在loss函数前面加上负号。
        """
        with tf.GradientTape() as tape:
